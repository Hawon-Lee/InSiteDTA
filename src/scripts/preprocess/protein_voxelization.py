...
class ProteinVoxelizer:
    ...
    def read_protein_features(self, pdb_file: str) -> list[dict]:
        ...
        obConversion.AddOption("c", obConversion.INOPTIONS)
        mol = openbabel.OBMol()
    ...
    def voxelize_gpu_v2(
        ...
        esm_data: dict = None,  # {"ca_coords": (N,3), "embeddings": (N,D)}
        pca_model: dict = None  # {"mean": ..., "components": ...} for dim reduction
        ...
        # ESM-C embedding voxelization (CA 좌표 기준 Gaussian density)
        if esm_data is not None and len(esm_data["ca_coords"]) > 0:
            esm_coords = esm_data["ca_coords"].astype(np.float32)
            esm_embeds = esm_data["embeddings"].astype(np.float32)
            if pca_model is not None:
                esm_embeds = (esm_embeds - pca_model["mean"]) @ pca_model["components"].T
            esm_dim = esm_embeds.shape[1]
            esm_coords_t = torch.tensor(esm_coords, device=device, dtype=torch.float32)
            esm_feats_t = torch.tensor(esm_embeds, device=device, dtype=torch.float32)
            flat_esm = torch.zeros((self.n_voxels**3, esm_dim), device=device)
            for i in range(0, esm_coords_t.shape[0], batch_size):
                bc = esm_coords_t[i : i + batch_size]
                bf = esm_feats_t[i : i + batch_size]
                dists = torch.cdist(bc, flat_voxel_centers)
                mask = dists <= r_cutoff
                densities = torch.exp(-dists**2 / 2) * mask.float()
                flat_esm += (densities.T @ bf)
            esm_voxel = flat_esm.view(self.n_voxels, self.n_voxels, self.n_voxels, esm_dim)
            voxel = torch.cat([voxel, esm_voxel], dim=-1)
