...
def get_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory containing pre-extracted ESM embedding pkl files (*_esm.pkl)",
    )
    parser.add_argument(
        "--pca_path",
        type=str,
        default=None,
        help="Path to PCA model pkl for ESM dim reduction (None = use full dim)",
    )
...
def voxelize_protein(
    ...
    if esm_dir is not None and not os.path.isdir(esm_dir):
    ...
    for pdb_id in tqdm(pdb_id_ls, desc="2. Preparing proteins"):
        ...
        if esm_dir is not None:
            ...
            if os.path.exists(esm_path):
                ...
                print(f"[Warning] ESM file missing for '{pdb_id}': {esm_path}")
